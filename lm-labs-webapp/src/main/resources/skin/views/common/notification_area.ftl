...
<#if Document.type != "Site" && Common.siteDoc(Document).getSite(Context.coreSession).getHomePageRef() == Document.id >
...
<#else>
    ...
    <#if Common.siteDoc(Document).getSite(Context.coreSession).getHomePageRef() == Document.id || Document.type == "Site" >
