...
<div class="bloc ${divClass}" id="${divId}" >
  ...
  <#list pageProvider.setCurrentPage(0) as doc >
    <li>
      ...
      <a href="${Context.modulePath}/${sd.getParentPagePath(Context.coreSession)}" title="/${sd.getParentPagePath(Context.coreSession)}<#if doc[tooltipDocProp] &gt; 0> - ${doc[tooltipDocProp]}</#if>" >
