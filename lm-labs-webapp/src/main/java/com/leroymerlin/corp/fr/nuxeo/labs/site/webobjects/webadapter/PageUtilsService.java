...
public class PageUtilsService extends DefaultAdapter {
    ...
    @POST
    @Path("move")
    public Response doAdminMove(@FormParam("source") String sourceId, @FormParam("destination") String destinationId, @FormParam("redirect") String redirect, @FormParam("view") String view)
            throws ClientException {
        DocumentModel doc = this.getTarget().getAdapter(DocumentModel.class);
        DocumentModel source = doc.getCoreSession().getDocument(new IdRef(sourceId));
        ...
        try {
            DocumentModel move = doc.getCoreSession().move(source.getRef(), destination.getRef(), null);
            doc.getCoreSession().save();
            if (BooleanUtils.toBoolean(redirect)) {
                return redirect(getPath() + viewUrl + "?message_success=label.admin.page.moved");
            } else {
                return Response.ok()
                        .entity(move.getAdapter(Page.class).getTitle())
                        .build();
            }
        } catch (Exception e) {
            if (BooleanUtils.toBoolean(redirect)) {
                return redirect(getPath() + viewUrl + "?message_error=" + e.getCause());
            } else {
    ...
    @POST
    @Path("copy")
    public Response doAdminCopy(@FormParam("source") String sourceId, @FormParam("destination") String destinationId, @FormParam("redirect") String redirect, @FormParam("view") String view)
            throws ClientException {
        DocumentModel doc = this.getTarget().getAdapter(DocumentModel.class);
        DocumentModel source = doc.getCoreSession().getDocument(new IdRef(sourceId));
        DocumentModel destination = doc.getCoreSession().getDocument(new IdRef(destinationId));
        ...
        try {
            DocumentModel copy = doc.getCoreSession().copy(source.getRef(), destination.getRef(), null);
    ...
    @POST
    @Path("rename")
    public Response doAdminRename(@FormParam("source") String sourceId, @FormParam("newTitle") String title, @FormParam("redirect") String redirect, @FormParam("view") String view)
            throws ClientException {
        DocumentModel doc = this.getTarget().getAdapter(DocumentModel.class);
        DocumentModel source = doc.getCoreSession().getDocument(new IdRef(sourceId));
        String viewUrl = "";
        ...
        if (BooleanUtils.toBoolean(redirect)) {
            return redirect(getPath() + viewUrl + "?message_success=label.admin.page.renamed");
        } else {
