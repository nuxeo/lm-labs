...
public class PageResource extends DocumentObject {
    ...
    protected Response addContent(String name, PageCreationLocation location, boolean overwrite) throws ClientException {
        ...
        final DocumentModel newDoc = DocumentHelper.createDocument(ctx, parent, name);
        
        if (newDoc.getType().equals(LabsSiteConstants.Docs.PAGEFORUM.type())) {
            UnrestrictedSessionRunner runner = new UnrestrictedSessionRunner(newDoc.getCoreSession()){
                @Override
                public void run() throws ClientException {
                    SecurityData data = SecurityDataHelper.buildSecurityData(newDoc);
                    data.addModifiablePrivilege(SecurityConstants.MEMBERS, SecurityConstants.ADD_CHILDREN, true);
                    
                    SecurityDataHelper.updateSecurityOnDocument(newDoc, data);
                    session.save();
                }
