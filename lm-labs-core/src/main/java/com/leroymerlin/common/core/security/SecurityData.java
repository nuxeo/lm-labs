...
public class SecurityData implements Serializable {
        ...
        if (grant) {
            ...
            if (null != currentDocGrant.get(principalName)) {
                ...
                if (shouldAddPermission) {
                    currentDocGrant.get(principalName)
                            .add(permissionName);
                }
            } else {
            ...
            if (null != currentDocDeny.get(principalName)) {
                ...
                if (shouldAddPermission) {
                    currentDocDeny.get(principalName)
                            .add(permissionName);
                }
            } else {
                // add the user and create a new list of rights for him
    ...
    public void addUnModifiablePrivilege(String principalName,
        ...
        if (grant) {
            ...
            if (null != parentDocsGrant.get(principalName)) {
                ...
                if (shouldAddPermission) {
                    parentDocsGrant.get(principalName)
                            .add(permissionName);
                }
            } else {
                // add the user and create a new list of rights for him
            ...
            if (null != parentDocsDeny.get(principalName)) {
                ...
                if (shouldAddPermission) {
                    parentDocsDeny.get(principalName)
                            .add(permissionName);
                }
            } else {
                // add the user and create a new list of rights for him
    ...
    /**
     * Blocks the right inheritance by adding ad Deny Everything to Everyone at
     * the current document level
     * 
     * @param value
     *            true if we want to block inheritance
     * @param currentUser
     *            if not null, adds Everything role to this user
     */
    @SuppressWarnings("deprecation")
    public void setBlockRightInheritance(Boolean value, String currentUser) {
        if (value) {// Block inheritance
            this.addModifiablePrivilege(SecurityConstants.EVERYONE,
                    SecurityConstants.EVERYTHING, false);

            if (this.getCurrentDocumentUsers() != null
                    && !this.getCurrentDocumentUsers()
                            .contains(currentUser)) {

                if (currentUser != null) {
                    this.addModifiablePrivilege(currentUser, SecurityConstants.EVERYTHING, true);
                }
                ...
                try {
                	...
                	for (String group : administratorsGroups) {
                		this.addModifiablePrivilege(group, SecurityConstants.EVERYTHING, true);
                	}
                } catch (Exception e) {
                	log.error(e, e);
                }
            ...
            this.removeModifiablePrivilege(SecurityConstants.EVERYONE,
                    SecurityConstants.EVERYTHING, false);
        }
    }

    /**
     * Return the status of the block inheritance business rule
     * 
     * @return
     */
    public Boolean isBlockRightInheritance() {
        if (getCurrentDocDeny() != null
                && getCurrentDocDeny().containsKey(SecurityConstants.EVERYONE)) {
            return getCurrentDocDeny().get(SecurityConstants.EVERYONE)
                    .contains(SecurityConstants.EVERYTHING);
        }
        return false;
    }
}
